	...
	if (subcommand === 'ls' || subcommand === 'list') {
        ...
        list.reduce((acc, i) => {
			...
			list.forEach(cert => {
				...
				getProcess().stdout.write(printf(spec, cn, time, expiration, autoRenew))
			})
		}
...
getProcess().on('uncaughtException', err => {
	handleError(err)
	exit(1)
})
...
function readConfirmation(cert, msg) {
	return new Promise(resolve => {
		...
		getProcess().stdout.write(`> ${msg}`)
		getProcess().stdout.write('  ' + tbl + '\n')

		getProcess().stdout.write(
			`${chalk.bold.red('> Are you sure?')} ${chalk.gray('[y/N] ')}`
		)

		getProcess().stdin
			.on('data', d => {
				getProcess().stdin.pause()
